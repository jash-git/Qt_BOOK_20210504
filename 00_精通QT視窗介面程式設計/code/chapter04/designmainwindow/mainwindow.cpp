...
void CMainWindow::iniDockWidget()
{
	CFindFileForm* findFileForm = new CFindFileForm;
        dockWidget = new QDockWidget(tr("尋找文件"), this);
...
void CMainWindow::iniConnect()
{
	...
	for (int i = 0; i < MaxRecentFiles; ++i) 
	{
        ...
        actPrint = menu_T->addAction(tr("列印文件"));
...
void CMainWindow::doCursorChanged()
{
	...
	int rowNum = textEdit->document()->blockCount();	
        label1->setText(tr("%1 頁 %3 列").arg(pageNum).arg(colNum));
}
...
bool CMainWindow::loadFile(const QString& fileName)
{
	...
	{
                QMessageBox::warning(this, tr("讀取文件"),
                                                tr("無法讀取文件 %1:\n%2.")
                                                .arg(fileName)
...
{
	QString fileName = 
                QFileDialog::getSaveFileName(this, tr("另存為"), curFile);
     if (!fileName.isEmpty())
	 {
		saveFile(fileName);	 
	 }
