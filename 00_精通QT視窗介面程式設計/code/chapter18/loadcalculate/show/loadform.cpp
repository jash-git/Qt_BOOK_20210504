...
void LoadForm::submitClickedSlot()
{
        ...
        housePriceEdit->setText(QString(tr("%1じ")).arg(calc->housePrice(),0,'f',2));
        leadEdit->setText(QString(tr("%1じ")).arg(calc->lead(),0,'f',2));
        totalPayEdit->setText(QString(tr("%1じ")).arg(calc->totalPay(),0,'f',2));
        totalInterestEdit->setText(QString(tr("%1じ")).arg(calc->totalInterest(),0,'f',2));
        initPayEdit->setText(QString(tr("%1じ")).arg(calc->initPay(),0,'f',2));
        totalMonthsEdit->setText(QString(tr("%1る")).arg(calc->totalMonths()));
        payEdit->setText(QString(tr("%1じ")).arg(calc->payPerMonth(),0,'f',2));
}
